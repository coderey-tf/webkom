...
@section('content')
    ...
    <main class="container mx-auto min-h-screen py-12 px-4 md:px-10">
        <div class="w-full flex justify-between flex-wrap items-center mb-8">
            ...
            <div class="w-full md:w-1/2 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <form action="{{ route('post.index') }}"
                    ...
                    <div class="w-full">
                        <input type="text" name="search" value="{{ request('search') }}"
                            class="w-full px-4 py-2 text-gray-900 rounded-md focus:outline-none" placeholder="Cari">
                    </div>
                    <div>
                        <button type="submit"
                ...
                <button onclick="togleDropdown()" class="relative px-4 py-1 bg-white rounded-md border border-gray-500">
        ...
        <div class="flex justify-between items-stretch flex-wrap gap-10 mb-6">
            @forelse ($posts as $post)
                <div class="flex flex-col rounded shadow-md w-full md:w-[30%]">
                    <div class="flex-shrink-0">
                        @if ($post->sampul)
                            <img class="h-48 w-full object-cover" src="{{ asset('storage/' . $post->sampul) }}"
                                alt="{{ $post->judul }}" />
                        @else
                    ...
                    <div class="flex-1 bg-white p-6 flex flex-col justify-between">
                        ...
                        <div class="mt-2">
                            <a class="text-blue-400 hover:text-blue-900 text-sm transition duration-150 ease-in-out group"
                                ...
                                <svg class="group-hover:hidden inline-block ml-1 w-2 h-2 stroke-2 stroke-current"
                                    viewBox="0 0 10 10" fill="none" aria-hidden="true">
                                    <g fill-rule="evenodd">
                                        <path d="M1 1l4 4-4 4"></path>
                                    </g>
                                </svg>
                                <svg class="hidden group-hover:inline-block ml-1 w-2 h-2 stroke-2 stroke-current"
                                    viewBox="0 0 10 10" fill="none" aria-hidden="true">
                                    <g fill-rule="evenodd">
                                        <path d="M0 5h7"></path>
